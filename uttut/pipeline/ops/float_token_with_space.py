...
FLOAT_TOKEN_WITH_SPACE_PATTERN = re.compile(r"(?<![\.\d])\d+\.\d+(?![\.\d])")
FLOAT_TOKEN_WITH_SPACE = " _float_ "
...
def _gen_forward_replacement_group(input_str: str) -> ReplacementGroup:
    ...
    while shift < len(input_str):
        match = FLOAT_TOKEN_WITH_SPACE_PATTERN.search(input_str, shift)
        if match is None:
            break
        replacement_group.add(
            start=match.start(),
            end=match.end(),
            new_value=FLOAT_TOKEN_WITH_SPACE,
        )
        shift = match.end()
...
def _forward_reduce_func(labels: List[int], output_size: int) -> List[int]:
    ...
    token_len = len(FLOAT_TOKEN_WITH_SPACE) - 2
    output_label = [0] + [common_label] * token_len + [0]
    return output_label


def _inverse_reduce_func(labels: List[int], output_size: int) -> List[int]:
...
class FloatTokenWithSpace(Operator):
